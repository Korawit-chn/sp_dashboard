...
async function markSeen(deviceID, { nowMs, bootID, uptimeSeconds, source, detail,
  ...
  // `AS incoming` rather than VALUES(). The VALUES() function is deprecated as
  // of MySQL 8.0.19 and removed in 9.0, and this statement runs on every
  // heartbeat from every Pi - so it was also the single loudest source of
  // deprecation warnings in the server log. Needs 8.0.19+, the release the
  // row-alias syntax arrived in.
  //
  // The target columns are qualified (DeviceStatus.bootID) so it stays obvious
  // which side of the upsert each half of the COALESCE comes from.
  await pool.execute(
    `INSERT INTO DeviceStatus
       (deviceID, lastHeartbeat, bootID, bootAt, connectionStatus,
        lastSyncAt, lastSyncOffsetMs, lastSyncRttMs)
     VALUES (?, ?, ?, ?, 'ONLINE', ?, ?, ?) AS incoming
     ON DUPLICATE KEY UPDATE
       lastHeartbeat = incoming.lastHeartbeat,
       bootID = COALESCE(incoming.bootID, DeviceStatus.bootID),
       bootAt = COALESCE(incoming.bootAt, DeviceStatus.bootAt),
       connectionStatus = 'ONLINE',
       lastSyncAt = COALESCE(incoming.lastSyncAt, DeviceStatus.lastSyncAt),
       lastSyncOffsetMs = COALESCE(incoming.lastSyncOffsetMs, DeviceStatus.lastSyncOffsetMs),
       lastSyncRttMs = COALESCE(incoming.lastSyncRttMs, DeviceStatus.lastSyncRttMs)`,
    [
      deviceID,
