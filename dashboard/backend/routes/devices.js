...
router.post('/deviceEvent', asyncRoute(async (req, res) => {
  ...
  // /heartbeat already answers this case with tracked:false and writes nothing.
  // This route inserted the event anyway, with deviceID NULL - a row that
  // belongs to no device, cannot be found through GET /api/deviceEvents?deviceID=,
  // and quietly accumulates every time an unregistered Pi shuts down. The two
  // routes resolve identity by the same rule, so they should agree on what to
  // do when it fails.
  if (deviceID == null) {
    return res.status(404).json({
      success: false,
      message: 'unknown device - register a sensor or actuator on it first'
    });
  }
...
router.get('/deviceEvents', asyncRoute(async (req, res) => {
  // Clamped at BOTH ends. Only the ceiling was capped, so ?limit=-5 reached
  // MySQL as `LIMIT -5` and came back as a 500 syntax error.
