...
router.post('/actuatorState', asyncRoute(async (req, res) => {
  ...
  // affectedRows was never checked, so a relay reporting to an actuatorID that
  // no longer exists - the normal state of affairs for a few minutes after the
  // database is rebuilt, and permanently if that Pi is never restarted - was
  // answered success:true forever while nothing at all was recorded.
  //
  // affectedRows is a sound existence test HERE specifically because
  // statusUpdatedAt = NOW(6) changes on every call: mysql2 reports rows
  // actually CHANGED, so a matching row whose state was already correct would
  // otherwise report 0 and produce a false 404.
  if (result.affectedRows === 0) {
    return res.status(404).json({
      success: false,
      message: `no actuator ${actuatorID} - re-register`
    });
  }
