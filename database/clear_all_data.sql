...
-- (SchemaVersion is NOT here on purpose - see the header.)
...
--   SamplingConfig  - BOTH intervals go. GET /api/schedule then falls back to
--                     the code defaults, 5 s sampling and a 60 s upload, and a
--                     Pi keeps whatever it last polled until it polls again -
--                     so the dashboard and the fleet disagree until then, with
--                     nothing on screen saying so. Set both again from the
--                     dashboard if you had them on something else.
