...
-- carries the full M001-M007 migration history. It is the only path that
-- preserves data. Back up before either:
--
--     mysqldump -u root -p sensor_dashboard_v3af > full_backup.sql
-- ----------------------------------------------------------------------------
--
-- The M-numbers in the comments below (M004, M005, M006, M007) name migrations in
...
-- starts life at v3.6, so Section 2 stamps M001-M007 as already applied: they
...
CREATE TABLE Location (
    ...
    -- findOrCreate() in helpers.js resolves a location by NAME, using an
    -- upsert: INSERT ... ON DUPLICATE KEY UPDATE. Without this key there is no
    -- duplicate for that clause to catch, so the upsert degrades into a plain
    -- INSERT and every registration adds another "Inside Dome" row.
    --
    -- That is not just clutter. The follow-up SELECT takes rows[0] with no
    -- ORDER BY, so once duplicates exist the locationID a sensor receives is
    -- not stable - and uq_sensor_identity (deviceID, typeID, locationID) then
    -- reads a shifted locationID as a DIFFERENT sensor, splitting one physical
    -- sensor's readings across two sensorIDs.
...
CREATE TABLE SensorType (
    ...
    -- NOT NULL matters as much as the UNIQUE below: MySQL treats every NULL in
    -- a unique key as distinct, so a nullable column would let findOrCreate()
    -- insert unlimited NULL-named rows - the same trap the locationName comment
    -- in routes/actuators.js warns about.
    sensorType VARCHAR(50) NOT NULL,

    -- Same reason as uq_location_name above: findOrCreate('SensorType', ...)
    -- is an upsert, and an upsert with no unique key to collide on just
    -- inserts.
    CONSTRAINT uq_sensortype_name UNIQUE (sensorType)
);
...
INSERT INTO SchemaVersion (migrationID, description) VALUES
  ...
  ('M007', 'Location.locationName and SensorType.sensorType UNIQUE - findOrCreate() upserts need a key to collide on');
...
--   SELECT VERSION();                  -- need 8.0.19+ (row-alias upsert in
--                                      -- deviceState.js; 8.0.12+ for INSTANT)
...
-- (7 rows). Anything else means this file did not finish.
